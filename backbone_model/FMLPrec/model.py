...
class BasicFMLPrec(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.item_emb = nn.Embedding(config["n_items"]+1, self.config["hidden_units"], padding_idx=0)
    ...
    def forward(self, interaction_list, interaction_mask, neg_list):
        ...
        batch_size, seq_length, _ = input_logs_embs.size()
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_logs_embs.device)  # [seq_len]
        position_ids = position_ids.unsqueeze(0).expand(batch_size, seq_length)  # [B, L]
        position_embs = self.position_embeddings(position_ids)  # [B, L, D]
        input_logs_embs = input_logs_embs + position_embs
    ...
    def predict(self, interaction_list, interaction_mask, item_indices):
        ...
        prec_vec = self.item_encoder(input_logs_embs,
                                    log_mask,
                                    output_all_encoded_layers=True)[-1][:, -1, :]
