    ...
    def train(self, path=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            if (epoch + 1) % self.config["val_interval"] == 0:
                ...
                # hr, ndcg, mrr = self.test_model()

        print(f"Validation Best - Hit Ratio@{self.config['top_k']}:{best_metrics:.4f}")
...
class SASrecTrainer(BasicTrainer):
    ...
    def _create_dataset(self, path):
        # self.dataset = SASDataset(path, self.config["LLM"], self.config, has_fake_user=True, max_len=self.config["max_interaction"])
    ...
    def _train_epoch(self, epoch):
        ...
        end_t = time.time()
        current_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())  # Get current time in string format
        loss_text = f"{current_time} - Epoch {epoch}: Rec Loss: {epoch_loss:.4f}"
    ...
    #         # hr, ndcg = self._eval_rec(org_seq, seq, mask, pos, tag="val")
    #         hr_all += hr
    #         ndcg_all += ndcg
    ...
    #         cnt += len(pos)
    #         hr, ndcg = self._eval_rec(org_seq, seq, mask, pos)
    #         # hr, ndcg = self._eval_rec(org_seq, seq, mask, pos, tag="test")
    #         hr_all += hr
    #         ndcg_all += ndcg
    ...
    def _eval_model(self, epoch):
        ...
        hr_all = 0
        ndcg_all = 0
        mrr_all = 0  # 新增：用于累计 MRR
        for batch_data in self.dataloader:
            org_seq, seq, mask, pos = self.dataset.get_val_batch(batch_data)
            hr, ndcg, mrr = self._eval_rec(org_seq, seq, mask, pos)  # 修改：接收 MRR
            hr_all += hr
            ndcg_all += ndcg
            mrr_all += mrr  # 新增：累计 MRR
        
        avg_hr = hr_all / self.dataset.n_users
        avg_ndcg = ndcg_all / self.dataset.n_users
        avg_mrr = mrr_all / self.dataset.n_users  # 新增：计算平均 MRR
        print(f"Validation at epoch {epoch} - Hit Ratio@{self.config['top_k']}: {avg_hr:.4f}, NDCG@{self.config['top_k']}: {avg_ndcg:.4f}, MRR@{self.config['top_k']}: {avg_mrr:.4f}")  # 修改：打印 MRR
    ...
    def test_model(self, model_path=None):
        ...
        avg_hr = hr_all / cnt
        avg_ndcg = ndcg_all / cnt
        avg_mrr = mrr_all / cnt  # 新增：计算平均 MRR
        print(f"Test - Hit Ratio@{self.config['top_k']}: {avg_hr:.4f}, NDCG@{self.config['top_k']}: {avg_ndcg:.4f}, MRR@{self.config['top_k']}: {avg_mrr:.4f}")  # 修改：打印 MRR


        if self.config["inject_user"]:
            self.test_attack()

        return avg_hr, avg_ndcg, avg_mrr  # 修改：返回 MRR
    ...
    def _eval_rec(self, org_seq, seq, mask, pos):
        ...
        all_logits = all_logits[:, 1:]  # 去掉 padding item（假设 index 0 是 padding）
        ...
        for user_idx, item_idx in enumerate(pos):
            rank = (sorted_indices[user_idx] == item_idx - 1).nonzero().item() + 1  # 找到正样本的排名
            if rank <= self.config["top_k"]:
                HR += 1
                NDCG += 1 / np.log2(rank + 1)
                MRR += 1 / rank  # 新增：计算 MRR 的倒数排名贡献

        return HR, NDCG, MRR  # 修改：返回 MRR
    ...
    def _eval_rec(self, org_seq, seq, mask, pos):
        ...
        # Get all possible item indices (excluding padding, assuming index 0 is padding)
        all_possible_idx = list(range(1, self.dataset.n_items + 1))

        with torch.no_grad():
            for user_idx, positive_item in enumerate(pos):
                # Randomly sample 99 negative items
                # negative_items = random.sample(all_possible_idx, 9)
                negative_items = random.sample(all_possible_idx, 99)
                # Ensure the positive item is included in the evaluation set
                if positive_item not in negative_items:
                    negative_items.append(positive_item)
                sampled_idx = negative_items

                # If using LLM embeddings, convert indices to embeddings
                if self.config.get("use_LLM", False):
                    sampled_idx = self.dataset.get_labels_emb(sampled_idx)

                # Predict logits for the sampled items
                logits = self.model.predict(seq[user_idx:user_idx+1], mask[user_idx:user_idx+1], sampled_idx)

                # Mask items that appear in the original sequence
                for i in range(logits.size(0)):
                    for item in org_seq[user_idx]:
                        if item in sampled_idx:
                            logits[i, sampled_idx.index(item)] = float('-inf')

                # Sort logits to get ranking
                _, sorted_indices = logits.sort(dim=1, descending=True)

                # Find the rank of the positive item
                positive_item_idx = sampled_idx.index(positive_item)
                rank = (sorted_indices[0] == positive_item_idx).nonzero().item() + 1

                # Update metrics if the positive item is in top-k
                if rank <= self.config["top_k"]:
                    HR += 1
                    NDCG += 1 / np.log2(rank + 1)
                    MRR += 1 / rank
    ...
    # def _eval_rec(self, org_seq, seq, mask, pos):
    # # def _eval_rec(self, org_seq, seq, mask, pos, tag):
    #     self.model.eval()
    #     all_idx = list(range(self.dataset.n_items + 1))
    #     if self.config.get("use_LLM", False):
    #         all_idx = self.dataset.get_labels_emb(all_idx)
    ...
    #     all_logits = all_logits[:, 1:]  # 去掉padding item（假设index 0是padding）
    ...
    #     # # 打印前几个样本的预测和真实标签（调试用）
    #     # if tag == "test" :
    #     #     num_print = min(100, sorted_indices.size(0))
    #     #     top_k = self.config["top_k"]
    #     #     for i in range(num_print):
    #     #         topk_pred = sorted_indices[i][:top_k].tolist()
    #     #         true_item = pos[i] - 1  # 标签值，去掉padding偏移
    #     #         input_seq = seq[i]
    #     #         org_input_seq = org_seq[i]
    #     #         masked_seq = [token for token, m in zip(input_seq, mask[i]) if m] if mask is not None else input_seq

    #     #         print(f"\n[Sample {i}]")
    #     #         print(f"  Original Sequence: {org_input_seq}")
    #     #         print(f"  Masked Sequence:   {masked_seq}")
    #     #         print(f"  Ground Truth Item: {true_item}")
    #     #         print(f"  Top-{top_k} Predictions: {topk_pred}")
    #     #         print(f"  {'✔️ Hit!' if true_item in topk_pred else '❌ Miss!'}")
